...
**`DhanBroker`** (`src/strategy/execution/dhan.rs`). A live `ExecutionTarget` around `Arc<DhanClient>`. `execute` delegates to `execute_with_meta(order, "", "")`. `execute_with_meta` places the order, refreshes the realized-loss cache, and **always appends a `TradeLogEntry` to the immutable JSONL trade log** — this is the single authoritative write path; no other code writes to the log. `get_positions` reads `/positions`, `available_cash` returns `f64::MAX` because this Dhan API surface does not expose free cash, and `realized_loss` is backed by a polling cache updated every 10 s and immediately after each order. `DhanBroker` also carries `session_context: Arc<RwLock<Option<SessionContext>>>` (set by `LiveSession::start`, cleared on stop) so the trade log entry is populated with the current strategy id, name, and mode without the broker needing to know anything about the session lifecycle.
...
- `execution.rs` — Two `ExecutionApi` implementations live here. `NoopExecutionApi` is the default when no live session is running — `submit_order` / `cancel_order` return `PluginError::ApiError` and `positions` returns an empty vec. `ReadOnlyLiveExecutionApi` swaps in once a session is live; it exposes `positions()` (bridging async → sync via `tokio::task::block_in_place + Handle::block_on`) but rejects `submit_order` / `cancel_order` with a Phase-7-specific error message. The runtime handle is captured at host-factory construction time (in `src-tauri/src/main.rs`) so plugin callbacks running on non-tokio threads don't crash on `Handle::current()`. **Phase 7 deliberately does not expose live order submission to plugins** — every real order must originate from the strategy engine so `MAX_DAILY_LOSS`, market-hours, pause, and trade-log context gates cover it. A properly gated plugin order gateway is Phase 8 work.
