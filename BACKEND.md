...
**Host functions** — the Rhai runtime bridges the **full** capability surface (registered in `register_host_functions`, called from `on_load`):

- `log_debug` / `log_info` / `log_warn` / `log_error` — ungated; route through the host's `LogApi`. The Tauri host factory wires a `RateLimitedFileLog` so every call is checked against a per-plugin token-bucket and dropped (with a per-minute summary line) if the plugin is spamming; the CLI path keeps using `NamespacedLog` (no plugins are loaded from the CLI, so rate limiting is unnecessary).
- `storage_get(key)` / `storage_set(key, val)` / `storage_delete(key) -> bool` / `storage_list_keys(prefix) -> array` — `Storage` capability; calls the underlying synchronous `StorageApi` and decodes `Vec<u8>` as UTF-8.
- `notify_info` / `notify_warning` / `notify_error`, `register_panel(id, title, route) -> bool`, `emit_panel_data(panel_id, json_string) -> bool` — `UiPanels` capability; `emit_panel_data` parses the JSON string and downcasts the `UiApi` to `TauriUiApi` (same path the WASM runtime uses).
- `submit_order(symbol, side, qty, order_type, price) -> order_id`, `cancel_order(order_id) -> bool`, `get_positions() -> array` — `Execution` capability. `submit_order` / `cancel_order` are async on the trait and are bridged with `tokio::task::block_in_place(|| Handle::current().block_on(..))`; `qty <= 0` and unknown side/order_type return `""`. `positions()` is sync and called directly.
- `get_latest_candle(symbol) -> map|()` — `MarketData` capability; `latest_candle` is sync but its impl calls a bare `block_on`, so it is wrapped in `block_in_place`.
- `register_indicator(name, fn_ptr)`, `register_metric(name, fn_ptr) -> bool`, `register_keyword(keyword, fn_ptr) -> bool`, `schedule(cron, fn_ptr) -> handle` / `cancel_schedule(handle) -> bool`, `subscribe_event(filter, fn_ptr) -> handle` — the callback-based registries (`Indicators` / `Analytics` / `DslExtension` / `Scheduler` / `Events` capabilities). Each closure captures the shared engine cell + `Arc<AST>` + a clone of the `FnPtr` and dispatches the user's Rhai function later (indicator eval, metric eval, DSL keyword eval, cron tick, event delivery). On any error or wrong-typed return the callback yields a benign fallback (`NaN` / `false` / no-op). `register_indicator` still downcasts via `as_any()` to `SharedIndicatorRegistry::register_fn` to carry the `PluginId` for dedup.

**Lifecycle wiring** — `RhaiPlugin::on_load` builds a **fresh** hardened engine (`build_hardened_engine`), compiles `self.source_path` with `engine.compile_file`, registers all host functions on the uniquely-owned `&mut Engine`, wraps it in `Arc`, populates the shared cell with a `Weak` handle, then `call_fn`s `on_load` (if defined). `EvalAltResult::ErrorFunctionNotFound` is swallowed; any other error maps to `PluginError::LoadFailed`. `on_enable` / `on_disable` follow the same pattern mapping to `PluginError::ApiError`. `on_unload` invokes the script's `on_unload` (errors ignored) and drops `self.host` and `self.ast`.

**Engine-sharing fix (critical).** The old `on_load` cloned the engine `Arc` and then tried `Arc::get_mut` to recover a `&mut Engine` for host-function registration. Because a live clone existed, `get_mut` returned `None` on **every** load and `RhaiPlugin::on_load` always failed with "engine Arc unexpectedly shared" — no test exercised the Rhai runtime end-to-end so it shipped. The fix builds a fresh engine inside `on_load`, registers on it while uniquely owned, then wraps it in `Arc`. Callback closures reach the engine later through a lazily-filled `Arc<OnceLock<Weak<Engine>>>` cell: the engine owns the closures, so the closures hold only a `Weak` back to it (avoiding a reference cycle), and every callback becomes a no-op after the plugin is unloaded and the engine `Arc` drops. `FnPtr` is `Send + Sync` because rhai is built with the `sync` feature. Guarded by `rhai_plugin_loads_and_drives_host_fns_end_to_end`.
...
- `log_debug(ptr, len)` — ungated; mirrors `log_info` at the debug level.
- `storage_delete(key_ptr, key_len) -> i32` — `Storage` capability; returns `0` on success, `-1` on permission denied / IO error.
...
- `register_panel(id_ptr, id_len, title_ptr, title_len, route_ptr, route_len) -> i32` — `UiPanels` capability; `0` on success, `-1` on error.
...
- `submit_order(sym_ptr, sym_len, side, qty, order_type, price, out_ptr, out_max, out_len_ptr) -> i32` — `Execution` capability; `side` is `0` = Buy / `1` = Sell, `order_type` is `0` = Market / `1` = Limit (price used only for Limit). Async on the trait, bridged with `block_in_place`. Returns `0` (order id written), `1` (out buffer too small — required length written to `out_len_ptr`, re-call with a bigger buffer), or `-1` (error / bad args / `qty <= 0`).
- `cancel_order(id_ptr, id_len) -> i32` — `Execution` capability; async, bridged with `block_in_place`. `0` on success, `-1` on error.
- `get_positions(out_ptr, out_max, out_len_ptr) -> i32` — `Execution` capability; writes a serde_json array of `{symbol, quantity, averagePrice}`. `0`/`1`/`-1` per the string-result ABI.
- `get_latest_candle(sym_ptr, sym_len, out_ptr, out_max, out_len_ptr) -> i32` — `MarketData` capability; writes a serde_json object `{open, high, low, close, volume, timestampMs}`. `latest_candle` is sync but calls a bare `block_on`, so it is wrapped in `block_in_place`. `0`/`1`/`-1` per the string-result ABI.

String-returning host fns share the `write_string_result(out_ptr, out_max, out_len_ptr, s)` helper ABI: the required byte length is always written to `out_len_ptr`, the bytes are copied only if they fit within `out_max`, and the return code is `0` (fit) / `1` (too small).

**Deferred for WASM: callback-based capabilities.** Custom indicators, analytics metrics, DSL keywords, cron tasks, event subscriptions, and market-data tick subscriptions all require the host to call *back into* the guest module later, on another thread. wasmtime's `Store` is neither `Sync` nor re-entrant, so bridging them would need a message-queue / re-entrant-store redesign; they are intentionally NOT exposed (and faked nowhere). The Rhai runtime bridges the full callback surface because its engine is `Sync` under the `sync` feature.

**Async bridge.** `StorageApi::read` / `write` are synchronous (the `async_trait` is forward-looking) and are called directly; the async `ExecutionApi::submit_order` / `cancel_order` and the bare-`block_on` `MarketDataApi::latest_candle` are driven through `tokio::task::block_in_place(|| Handle::current().block_on(...))`, which requires a multi-thread runtime (Tauri default).

**Lifecycle wiring** — `WasmPlugin::on_load` reads the artifact, compiles it with `Module::new`, builds the linker, constructs the store with the inline `MemoryLimitState`, arms the per-call epoch deadline, spawns the `EpochWatchdog`, instantiates, and dispatches `_algomln_on_load` if exported (the deadline is re-armed inside `call_lifecycle` so subsequent lifecycle calls also see a fresh budget). `on_enable` / `on_disable` follow the same pattern for `_algomln_on_enable` / `_algomln_on_disable`. `on_unload` calls `_algomln_on_unload` (errors ignored), drops the `EpochWatchdog` (which stops + joins the watchdog thread), and drops both the store and the instance, releasing all memory back to wasmtime.
