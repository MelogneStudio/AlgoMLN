...
- `execution.rs` — Three `ExecutionApi` implementations live here. `NoopExecutionApi` is the default when no live session is running — `submit_order` / `cancel_order` return `PluginError::ApiError` and `positions` returns an empty vec. `ReadOnlyLiveExecutionApi` swaps in once a session is live; it exposes `positions()` (bridging async → sync via `tokio::task::block_in_place + Handle::block_on`) but rejects `submit_order` / `cancel_order` with a Phase-7-specific error message. The runtime handle is captured at host-factory construction time (in `src-tauri/src/main.rs`) so plugin callbacks running on non-tokio threads don't crash on `Handle::current()`. The **`GatedLiveExecutionApi`** is the Phase 8 add (Task #4) — it re-runs every engine gate (market hours, broker stale, paused-for-entries, risk controls) per plugin `submit_order` call and routes the order through the same `execute_intent` path the engine uses, so a plugin order is indistinguishable from an engine order on the audit log. The Tauri host factory wires `GatedLiveExecutionApi` for active sessions; `ReadOnlyLiveExecutionApi` stays as the in-progress fallback so a plugin that does not opt-in to the `Execution` capability still gets position reads.
