...
| **C2 / H3.5** Phase 8 plugin order gateway | ⏭ Pending (Phase 8 Task #4) | Implement `GatedLiveExecutionApi` in `src/plugin/api/execution.rs` that re-runs market-hours + stale + pause + risk gates per call, then call `broker.execute_with_meta` through the same intent executor used by the live tick loop. Update `src-tauri/src/main.rs` host factory to wire it when a session is active. Update CLAUDE.md invariant #18. |
