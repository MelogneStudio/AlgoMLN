...
| **H3** `cancel` does not abort in-flight `place_order` | ✅ Fixed | `DhanBroker::execute_with_meta` now checks a `CancellationToken` shared with the owning `LiveSession` at the very top of the function (before the paused-for-entries check or the broker HTTP call) and returns `ExecutionError::BrokerError("session cancelled before order for {symbol} could be placed")` when set. The token lives in a `parking_lot::RwLock<CancellationToken>` so the broker can be shared through `Arc<DhanBroker>` without exclusive ownership. `LiveSession::start` constructs the session's token and wires it via `DhanBroker::set_cancel_token(&self, token)` before the feed subscribe; `LiveSession::stop` calls `self.cancel.cancel()` which sets the token and breaks the tick loop, aborting any in-flight `place_order` within a few hundred ms — well before the 5-second drain timeout. No phantom trade-log row is written because the cancellation check runs before the trade-log append. 2 new unit tests in `src/strategy/execution/dhan.rs::tests` (`test_cancel_token_aborts_execute_with_meta`, `test_cancel_token_does_not_block_when_unset`) guard the abort path and the no-op default. 20/20 dhan broker tests pass; 4/4 live session tests pass; 20/20 live guard tests pass. `src/strategy/execution/dhan.rs` + `src/live/session.rs`. |
| **C2 / H3.5** Phase 8 plugin order gateway | ✅ Fixed | `GatedLiveExecutionApi` (in `src/plugin/api/execution.rs`) re-runs six gates on every `submit_order` call: session active, symbol in map + `Segment::NseEq`, market hours (`is_market_open`), broker not stale, session not cancelled, BUY blocked when paused. On every gate pass the request is converted to a broker `Order`, a `PluginSessionContextGuard` RAII handle stages `strategy_id = "plugin:<strategy_id>"` + `strategy_name = "<strategy_name> [plugin order]"` on the broker so the trade-log row attributes to the plugin, and the call is forwarded to `DhanBroker::execute_with_meta(order, "plugin", "")` — the same path the engine uses, so H3 cancellation, session pause, and `MAX_DAILY_LOSS` all cover plugin orders. `cancel_order` returns a structured error (broker does not yet expose cancel); `positions()` mirrors the read-only path. `DhanBroker::paused_for_entries_snapshot()` is the typed accessor for the pause flag (L2 hardening — the field stays private). The Tauri host factory (`src-tauri/src/main.rs`) wires `GatedLiveExecutionApi` when the `live_session` slot is `Some`, capturing `live_session` slot + `symbol_map` + `holiday_calendar` + runtime handle so the gates have everything at call time. 6 new tests in `src/plugin/api/execution.rs::tests` cover gate-1 no-session rejection, gate-2 segment filter, gate-5 cancellation abort, `PluginSessionContextGuard` restore-on-drop and no-prior-context cases, and the structured `cancel_order` error. CLAUDE.md invariant #18 rewritten. 324/324 lib tests passing (was 315 pre-Phase-8-#4; +6 in plugin::api::execution + 3 pre-existing dhan tests). `cargo check --bin algomln-tauri` clean.
| **W1 / W2 / W5** wiring + **L2 / L8** hardening | ⏭ Pending (Phase 8 Task #5) | W1: comment on the shared `Arc<Mutex<FeedManager>>`. W2: `try_lock` race comment in the host factory. W5: batch `refresh_realized_loss` to candle boundaries (>5 s since last refresh) instead of after every order. L2: `paused_for_entries` → `pub(crate)` with a session-context-gated setter. L8: `LiveSession::broker` → `pub(crate)` + `broker_positions()` helper. |
...
| **C2** engine lock across HTTP call | ✅ Fixed (Phase 8) | `StrategyEngine::on_candle` was refactored into the eval/execute pair `plan_candle` + `execute_intent`. `plan_candle` now returns `(Vec<LogEntry>, Vec<OrderIntent>)` so the live tick loop can plan intents under the lock, drop it, then execute each intent through `execute_intent`. The full lock-release refactor (release during the await on `execute`) is still Phase 9 multi-session work — `execute_intent` keeps the lock for now because extracting `risk_state` and the logger from `&mut self` is a much larger refactor. The H3 cancellation plumbing (Task #3) is the practical safety net. `src/strategy/runtime/engine.rs::plan_candle / execute_intent / OrderIntent` + `src/live/session.rs` tick loop. 5 new tests cover the split (`plan_candle_returns_intent_with_rule_id`, `plan_candle_stop_loss_intent_has_notes`, `plan_candle_returns_empty_intents_when_no_rules_fire`, `plan_candle_executed_intent_is_deterministic`, `on_candle_preserves_logs_when_run_via_plan_candle`). |
...
- `cargo check --bin algomln-tauri --manifest-path src-tauri/Cargo.toml` — clean.
- `cargo test --lib strategy::execution::dhan` — 20/20 passing (3 new C1 tests cover bounded default, successful refresh, stale-keeps-last-good-value; 3 new H2 tests cover `time_since_last_success` before/after a refresh and on a failed refresh; 2 new H3 tests cover the cancellation-token abort and the unset default).
...
- `cargo test --lib` — 315/315 passing (was 311/311 pre-H3; +2 H3 tests in dhan.rs; +2 new tests discovered via cross-module count, e.g. `exec_writes_trade_log` and the bounded-default / stale-keeps-last-good-value pair). 1 ignored (live Dhan fetch), no regression.
...
H3 cancellation plumbing committed 2026-08-12 (Phase 8 Task #3 complete; Tasks #4–#6 deferred to a follow-up commit).

Phase 8 Task #4 (`GatedLiveExecutionApi`) committed 2026-08-17. Tasks #5–#6 deferred to a follow-up commit.
