...
**9c. Single live session manager.** Phase 7 live execution is intentionally single-session. `AppState.live_session` holds `Arc<tokio::sync::Mutex<Option<Arc<LiveSession>>>>`. `src/live/session.rs::LiveSession` owns the strategy engine, Dhan broker, trade log, rolling candle history (capped at `MAX_CANDLE_HISTORY = 5000` entries — oldest candles are drained from the front on overflow), status, and cancellation token. It subscribes through the shared `FeedManager`, uses `src/live/candle_assembler.rs::CandleAssembler` to close 1-minute candles from ticks, appends each completed candle to history, and calls the same `StrategyEngine::on_candle` used by backtests/paper runs. **Paused sessions continue to assemble candles and run `on_candle` every minute** — only new *entry* (BUY) orders are suppressed, via `DhanBroker::paused_for_entries`. Stop-loss, take-profit, and risk-breach SELL orders always execute regardless of pause state. The tick loop also auto-pauses (same mechanism) when `DhanBroker::is_stale()` is true (realized-loss cache degraded after 3 consecutive failures) — session stays Paused until the user manually resumes once the cache recovers. `RecvError::Lagged` on the tick broadcast channel is non-fatal (`continue`); `RecvError::Closed` breaks the loop, sets status to `Failed("feed closed unexpectedly")`, and calls `SessionEventEmitter::emit_failed` with a `LiveSessionFailedPayload`. `SessionEventEmitter` is a trait injected into `LiveSession::start`; the Tauri binary wraps `AppHandle` in a `TauriSessionEmitter` impl, tests use `MockEmitter`, non-alert paths use `NoopEmitter`. Stopped sessions cancel the task. `LiveSession::start` writes a `SessionContext { strategy_id, strategy_name, mode: "live" }` onto `broker.session_context` before starting; `stop` clears it, so the broker always logs the correct session metadata regardless of which code path triggers `execute`.

**9d. Live-start safety gate layer.** No live order may ever be placed unless gates 1–8 pass via `src/live/guard.rs::LiveGuard::run_preflight`. The guard is the only path that issues a `PendingLiveToken`; the token is single-use, 90 s TTL, bound to a strategy id, and stored in `AppState.pending_live_token`. Gate order: paper-default (record's `mode` == Live), broker reachability (`get_positions` succeeds), symbol in map, segment == NseEq, market hours (09:15–15:30 IST Mon–Fri, NSE holiday calendar excluded), at least one risk control declared, `MAX_DAILY_LOSS` is `Some(_)` (hard, non-negotiable), broker not stale. Ack is not a gate — it picks `LiveGuardResult::Ok` vs. `RequiresAcknowledgment`; both variants carry a token. `request_live_start` issues the token and stores it; `confirm_live_start` validates + clears + starts session; `acknowledge_live_trading` is a pure file write with no token involvement. `confirm_live_start` holds the `live_session` lock across `LiveSession::start` so a concurrent `request_live_start` cannot slip a second session in. Hot path: `src/commands/live.rs`.
...
**18. Phase 7 — plugin order submission is disabled.** Live orders can only originate from the strategy engine so `MAX_DAILY_LOSS`, market-hours, session pause, and trade-log context gates cover every real order. `ReadOnlyLiveExecutionApi` (`src/plugin/api/execution.rs`) is the only `ExecutionApi` wired into plugin hosts in Phase 7; its `submit_order` / `cancel_order` return `PluginError::ApiError` with a Phase-7-specific message, while `positions()` returns the broker's live position snapshot. The Tauri `HostFactory` closure captures the `live_session` slot and the multi-thread tokio runtime handle so that plugin hosts constructed during an active session get `ReadOnlyLiveExecutionApi` (bridging the trait's sync `positions()` to the broker's async `get_positions` via `tokio::task::block_in_place + Handle::block_on`); hosts constructed when no session is active get `NoopExecutionApi`. A properly gated plugin order gateway that re-runs every engine gate is **Phase 8** work.
