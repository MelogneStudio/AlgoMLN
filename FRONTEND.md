...
The Rust backend also registers `get_trade_log`, which returns immutable live execution log entries newest first from `<app_data>/trade_log.jsonl`. The backend also stores a single active `LiveSession` slot in `AppState`, plus a `LiveGuard` safety-gate layer behind `request_live_start` / `confirm_live_start` / `acknowledge_live_trading` (Phase 7, Prompt 4). The preflight runs gates 1–8 (paper-default, broker reach, symbol/segment, market hours, risk controls, stale cache), issues a 90 s token, and either grants start or asks for one-time acknowledgment. **Phase 7 Prompt 5** adds the runtime-control IPC wrappers — `pause_live_strategy` / `resume_live_strategy` / `stop_live_strategy` / `get_live_status` — and the `LiveStatusWire` / `StopResult` / `RequestLiveStartResult` wire types in `src/types/live.ts`. The corresponding TS-side IPC wrappers (`pauseLiveStrategy`, `resumeLiveStrategy`, `stopLiveStrategy`, `getLiveStatus`, plus the existing `getTradeLog` / `requestLiveStart` / `confirmLiveStart` / `acknowledgeLiveTrading`) live in `src/types/tauri.ts` and follow the same `isTauri()`-guarded `invoke<T>(...)` pattern. The `LiveSessionFailedPayload` and `LiveSessionStoppedPayload` types describe the Tauri events the React app subscribes to. UI screens that drive a live session (status panel, start/pause/stop buttons, ack dialog) are added in a follow-up.

**Live-confirm modal UX (L4 audit).** The preflight step in `LiveConfirmModal` warns that sessions started in the last minute before 15:30 close may still attempt to place an order on the 15:30 candle (which the exchange rejects). This is consistent with `LiveGuard::is_market_open` only gating session *start*, not per-candle ticks. The H2 resume-during-stale error from the backend is surfaced verbatim in the toast/notifications system.
